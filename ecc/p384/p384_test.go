...
import (
	...
	"github.com/khulnasoft/recryptor/ecc/p384"
	"github.com/khulnasoft/recryptor/internal/test"
)

func TestIsOnCurveTrue(t *testing.T) {
	RecryptorCurve := p384.P384()
	k := make([]byte, 384/8)
	for i := 0; i < 128; i++ {
		_, _ = rand.Read(k)
		x, y := elliptic.P384().ScalarBaseMult(k)

		got := RecryptorCurve.IsOnCurve(x, y)
		want := true
		if got != want {
...
func TestAffine(t *testing.T) {
	...
	t.Run("Addition", func(t *testing.T) {
		for i := 0; i < testTimes; i++ {
			...
			gotX, gotY := RecryptorCurve.Add(X1, Y1, X2, Y2)
	...
	t.Run("Double", func(t *testing.T) {
		for i := 0; i < testTimes; i++ {
			...
			gotX, gotY := RecryptorCurve.Double(x, y)
...
func TestScalarBaseMult(t *testing.T) {
	...
	t.Run("0P", func(t *testing.T) {
		...
		for i := 0; i < len(k); i += 20 {
			gotX, gotY := RecryptorCurve.ScalarBaseMult(k[:i])
	...
	t.Run("kP", func(t *testing.T) {
		...
		for i := 0; i < testTimes; i++ {
			_, _ = rand.Read(k)
			gotX, gotY := RecryptorCurve.ScalarBaseMult(k)
...
func TestScalarMult(t *testing.T) {
	...
	t.Run("k=0", func(t *testing.T) {
		k := []byte{0x0}
		gotX, gotY := RecryptorCurve.ScalarMult(params.Gx, params.Gy, k)
		got := RecryptorCurve.IsAtInfinity(gotX, gotY)
		want := true
		if got != want {
	...
	t.Run("random k", func(t *testing.T) {
		for i := 0; i < testTimes; i++ {
			k, _ := rand.Int(rand.Reader, params.N)
			gotX, gotY := RecryptorCurve.ScalarMult(params.Gx, params.Gy, k.Bytes())
	...
	t.Run("wrong P", func(t *testing.T) {
		for i := 0; i < testTimes; i++ {
			...
			got := RecryptorCurve.IsOnCurve(x, y) && RecryptorCurve.IsOnCurve(RecryptorCurve.ScalarMult(x, y, k.Bytes()))
...
func TestCombinedMult(t *testing.T) {
	...
	for i := 0; i < testTimes; i++ {
		...
		gotX, gotY := RecryptorCurve.CombinedMult(X, Y, K1.Bytes(), K2.Bytes())
		if gotX.Cmp(wantX) != 0 {
...
func Example_p384() {
	// import "github.com/khulnasoft/recryptor/ecc/p384"
	// import "crypto/elliptic"
	recryptor := p384.P384()
	stdlib := elliptic.P384()

	params := recryptor.Params()
	K, _ := rand.Int(rand.Reader, params.N)
	k := K.Bytes()

	x1, y1 := recryptor.ScalarBaseMult(k)
